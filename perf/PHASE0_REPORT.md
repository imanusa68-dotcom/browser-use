...
## Итерация P6 — аудит всех `asyncio.sleep` в default_action_watchdog.py (2026-08-27)

Всего 27 вхождений (grep после P1–P5; номера строк актуальны на коммит a538b01).
Вердикты: **(a)** уже заменён P1–P5 (легаси-ветка осталась как fallback/при выключенном флаге);
**(b)** заменить безопасно; **(c)** оставить (обоснование добавлено в код: `# perf-audit: kept — …`).

| Строка | Контекст | Вердикт |
|---|---|---|
| 83 | `_raf_tick`: fallback-sleep, если rAF-evaluate упал/затроттлен; сохраняет семантику потолка | инфраструктура P2 — condition-wait с потолком, менять нечего |
| 672 | scroll в iframe: пауза на settle перед инвалидацией DOM-кэша | (c) kept — редкий путь, у кросс-origin iframe нет дешёвого rect-probe из родительской сессии |
| 915 | клик: легаси-пауза после scrollIntoViewIfNeeded | (a) P2 `fast_scroll_stability` → `_wait_element_rect_stable` |
| 968 | клик: JS-fallback для перекрытого элемента (beat для старта навигации) | (c) kept — деградированный путь, не горячий |
| 1046 | клик: JS-fallback для перекрытого элемента (второй сайт) | (c) kept — тот же rationale |
| 1083 | клик: легаси-пауза 50мс после mouseMoved | (a) P3 `fast_click` → 0 |
| 1102 | клик: press hold (80мс легаси) | (a) P3 → `click_press_duration_ms` (20мс) |
| 1133 | checkbox/radio: beat перед readback state | (c) kept — верификация тоглов, защищает от ложного «unchanged» → лишнего re-click |
| 1153 | checkbox: recovery после несработавшего CDP-тогла | (c) kept — error-recovery, редкий |
| 1194 | клик: JS-fallback после исключения CDP-клика | (c) kept — error-recovery |
| 1278 | `_click_on_coordinate`: пауза после mouseMoved | (a) P3 → 0 |
| 1297 | `_click_on_coordinate`: press hold (50мс легаси) | (a) P3 → `click_press_duration_ms` |
| 1401 | посимвольный ввод на уровне страницы (без element node) | (c) kept — редкий путь, преднамеренный human-speed pacing |
| 1984 | ввод: легаси-пауза после scrollIntoViewIfNeeded перед фокусом | (a) P2 → rect-stability |
| 2147, 2190, 2236, 2253 | посимвольный ввод в элемент: паузы 1–5мс между key events | (a) P1 `fast_input` заменяет весь посимвольный путь одним insertText; эти sleep остаются ТОЛЬКО в легаси-арме и в fallback-е P1 (mask mismatch) — трогать нельзя, это и есть страховка P1 |
| 2263 | 50мс перед readback значения (settle форматтеров) | (a) P1 → 1 rAF-тик с потолком 100мс; легаси-ветка остаётся при выключенном флаге |
| 2653 | go_back: ожидание navigateToHistoryEntry | (c) kept — редкое действие, готовность страницы всё равно перепроверяется DOMWatchdog |
| 2682 | go_forward | (c) kept — тот же rationale |
| 2699 | refresh: ожидание reload | (c) kept — редкое действие; pending-часть покрыта P5 |
| 2721 | явный wait-action от LLM/пользователя | (c) kept — сон здесь и есть функция |
| 2935 | send_keys: 10мс между символами | (c) kept — шорткаты/несколько символов, не горячий; pacing защищает debounce-обработчики |
| 2944 | send_keys: 100мс после Enter (возможный form-submit) | (c) kept — один beat для старта навигации |
| 3422 | ARIA dropdown: ожидание разворачивания опций | (c) kept — редкое действие, нет универсального сигнала завершения анимации |
| 3854 | dropdown: retry внутри error-recovery выбора | (c) kept — one-shot recovery |

**Итог:** (a) — 11 вхождений (все горячие пути уже покрыты P1–P5); (b) — **0** значимых кандидатов:
оставшиеся sleep'ы сидят в редких/деградированных/recovery-путях, суммарный вклад в типичную задачу
< 100 мс (в трейсах бенча эти ветки не срабатывают ни разу на 18 прогонов), поэтому отдельная
мини-итерация с A/B не требуется; (c) — 15 вхождений, каждое получило обоснование `# perf-audit: kept`
прямо в коде. Принято по regression 4/4 (оба арма) + sanity SR — без полного A/B, что явно
зафиксировано здесь по правилу итерации P6.
